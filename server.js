...
// ── Sincronizar portfolio con balance REAL de Bybit ──────────────────────
async function syncPortfolio() {
  try {
    const d = await bybitGet("/v5/account/wallet-balance", {accountType:"UNIFIED"});
    if(d.retCode !== 0) { console.error("Sync error:", d.retMsg); return; }
    const coins = d.result.list[0]?.coin || [];

    // 1) USDT real disponible
    const usdtCoin = coins.find(c => c.coin === "USDT");
    bot.portfolio.usdt = usdtCoin ? parseFloat(usdtCoin.walletBalance) : 0;

    // 2) Por cada símbolo que el bot opera, revisar si hay holding real
    for(const sym of bot.symbols) {
      const base = sym.replace("USDT",""); // SOLUSDT -> SOL
      const coin = coins.find(c => c.coin === base);
      const qty = coin ? parseFloat(coin.walletBalance) : 0;
      const price = bot.prices[sym];

      // dust: umbral mínimo para considerar que "hay" posición (evita residuos)
      const minValue = 1; // USD
      const hasRealPosition = price ? (qty * price) > minValue : qty > 0;

      if(hasRealPosition && !bot.portfolio.positions[sym]) {
        // Hay holding real que el bot no tenía registrado (compra manual)
        bot.portfolio.positions[sym] = { qty, entry: price || qty, value: qty*(price||0) };
        console.log(`🔄 Sync: posición detectada en ${sym} (${qty})`);
      } else if(!hasRealPosition && bot.portfolio.positions[sym]) {
        // El bot tenía la posición pero ya no hay balance real (venta manual)
        delete bot.portfolio.positions[sym];
        console.log(`🔄 Sync: posición en ${sym} cerrada externamente, limpiada`);
      } else if(hasRealPosition && bot.portfolio.positions[sym]) {
        // Actualizar cantidad por si cambió (venta/compra parcial manual)
        bot.portfolio.positions[sym].qty = qty;
      }
    }
  } catch(e) {
    console.error("❌ Error syncPortfolio:", e.response?.data || e.message);
  }
}
...
// Orden manual
// Vender manualmente una posición abierta específica
app.post("/api/sell-position", async (req,res) => {
  const {symbol} = req.body;
  if(!symbol) return res.status(400).json({error:"Falta symbol"});
  if(!bot.portfolio.positions[symbol]) return res.status(400).json({error:`No hay posición abierta en ${symbol}`});
  try {
    const price = bot.prices[symbol];
    await executeOrder(symbol, "SELL", price);
    res.json({ok:true, message:`Venta ejecutada: ${symbol}`, portfolio:bot.portfolio});
  } catch(e){ res.status(500).json({error: e.response?.data || e.message}); }
});
...
app.post("/bot/start", async (req,res) => {
  ...
  // Ya no se usa capitalUSDT manual: el capital y las posiciones se leen
  // directamente de Bybit real al arrancar y en cada ciclo.
  await fetchMarket();
  await syncPortfolio();
  if(bot.portfolio.totalStart === 0) bot.portfolio.totalStart = bot.portfolio.usdt;

  bot.active = true;
  startLoop();
  res.json({ok:true, message:"Bot iniciado 24/7 en Bybit (balance real sincronizado)", config:{strategy:bot.strategy, riskPct:bot.riskPct, symbols:bot.symbols, capital:bot.portfolio.usdt}});
});
...
// Forzar sincronización manual con balance real de Bybit
app.post("/api/sync", async (req,res) => {
  try {
    await fetchMarket();
    await syncPortfolio();
    res.json({ok:true, portfolio:bot.portfolio, lastUpdate:bot.lastUpdate});
  } catch(e) { res.status(500).json({error: e.response?.data || e.message}); }
});
