...
async function executeOrder(sym, side, price) {
  ...
  if(side==="BUY") {
    ...
    // Usar todo el capital disponible si el trade calculado es menor a $5
    if(tradeUSD < 5) {
      console.log(`Ajustando monto de $${tradeUSD.toFixed(2)} a todo el capital: $${bot.portfolio.usdt.toFixed(2)}`);
    }
  }
  if(side==="SELL") {
  ...
  const actualTradeUSD = side==="BUY" ? Math.max(tradeUSD, Math.min(bot.portfolio.usdt, 15)) : 0;
  const qty = side==="BUY"
    ? parseFloat((actualTradeUSD/price).toFixed(6))
...
// Diagnóstico — prueba conexión a Binance
app.get("/api/test", async (_, res) => {
  try {
    const r = await axios.get("https://api.binance.com/api/v3/ticker/price?symbol=BTCUSDT", {timeout:10000});
    res.json({ok:true, btcPrice: r.data.price, msg:"Binance accesible desde Railway"});
  } catch(e) {
    res.json({ok:false, error: e.message, code: e.code, msg:"Error conectando a Binance"});
  }
});
